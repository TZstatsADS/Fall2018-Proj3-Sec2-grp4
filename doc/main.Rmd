...
Our baseline model runs cross-validation with 5 folds. 
...
It is also faster than bilinear interpolation which uses the weighted average of two translated pixel values for each output pixel value.
  Additionally, it is also faster than bicubic interpolation which uses the weighted average of four translated pixel values for each output pixel value.
  
  Our improved model is ... faster
  Time for constructing training features= 70.589 s 
  Our baseline models takes 14hrs 
