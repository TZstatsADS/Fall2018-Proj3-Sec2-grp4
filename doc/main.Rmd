...
```{r wkdir, eval=FALSE}
set.seed(2018)
setwd("/Users/fangqiouyang/Fall2018-Proj3-Sec2-grp4") 
...
if(run.cv.xg){
  par(mfrow=c(3,4))
  for(i in 1:12){
    u <- paste("../output/error",i,"_xg.RData",sep="")
    load(u)
    plot(depth.list,error[,1],type="o",col=1,xlab="depth",ylab="error rate",main="Cross Validation Error")
    for(i in 2:length(eta.list)){
      points(depth.list,error[,i],type="o",col=i)
    }
    legend("topright",fill=1:length(eta.list),legend=paste("eta=",eta.list),cex=0.75)
  }}
